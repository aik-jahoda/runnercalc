...
import { NameNumerology } from "../src/engine/numerologyName";
import { getMessage, MessageType } from "../src/engine/numberExplanation";
import { getNames } from "../src/engine/selectName";
...
export class NameSearch extends React.Component<{}, NameSearchState> {
  ...
  positiveChanged = (event: React.ChangeEvent<HTMLInputElement>) => {
    event.preventDefault();
    this.setState({
      minimumPositive: Number.parseInt(event.target.value) || 0,
      minimumEdit: event.target.value
    });
  };
  ...
  negativeChanged = (event: React.ChangeEvent<HTMLInputElement>) => {
    event.preventDefault();
    this.setState({
      maximumNegative: Number.parseInt(event.target.value) || 0,
      maximumEdit: event.target.value
    });
  };
  ...
  render() {
    const searchBox = (
      <div>
        {" "}
        Příjmení:{" "}
        <input onChange={this.surnameChanged} value={this.state.surname} />
        Minimum pozitivních:{" "}
        <input onChange={this.positiveChanged} value={this.state.minimumEdit} />
        Maximum negativních:{" "}
        <input onChange={this.negativeChanged} value={this.state.maximumEdit} />
        2 jména:{" "}
        <input
          onChange={this.secondNameChnanged}
          type="checkbox"
          checked={this.state.secondName}
        />
      </div>
    );
    ...
    const names =
      this.state.surname.length > 0
        ? getNames(
            this.state.maximumNegative,
            this.state.minimumPositive,
            this.state.surname,
